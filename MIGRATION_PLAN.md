...
- **Phase 7:** Single entry stylesheet `src/styles/main.css` (layers → tokens → legacy aliases → reset → base → components → density → app shell); `src/index.ts` and `.ladle/components.tsx` import it; `ThemeProvider` toggles **`dark` class** on `documentElement` for `.dark` token overrides.
- **Phase 8:** `scripts/export-tokens.cjs` (merged `:root` / `.dark` from token files + legacy alias mapping for Variables Pro); `scripts/export-tokens-figma.cjs` paths → `src/styles/tokens/*`; `npm run tokens` runs both (`.cjs` because `package.json` is `"type": "module"`).
...
- **Legacy CSS:** `src/styles/global.css` and `src/styles/components.css` are **not** imported; kept in tree for reference during review (can delete after sign-off).
