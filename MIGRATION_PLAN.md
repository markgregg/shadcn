...
- **Phase 8:** `scripts/export-tokens.cjs` reads the same CSS sources as the token pipeline and writes **`tokens/light.json`** + **`tokens/dark.json`** for Figma Variables import (`.cjs` because `package.json` is `"type": "module"`). Run `npm run tokens` after token changes.
