...
- **Phase 7:** Single entry stylesheet `src/styles/main.css` (layers → tokens → reset → base → components → density → app shell); `src/index.ts` and `.ladle/components.tsx` import it; `ThemeProvider` toggles **`dark` class** on `documentElement` for `.dark` token overrides.
